...
export interface ConnectionState {
  jira: ConnectionStatus & { baseUrl?: string; email?: string };
  servicenow: ConnectionStatus & { instanceUrl?: string; username?: string };
  llmProviders: Record<string, { isConfigured: boolean; model?: string }>;
}
...
class ConnectionManager {
  ...
  private async initializeJira(): Promise<void> {
    ...
    try {
      ...
      (global as any).defaultJiraConnection = { baseUrl, email: username, apiToken: apiKey };
   ...
   */
  private async initializeServiceNow(): Promise<void> {
    ...
    try {
      ...
      // Store in global for session reuse
      (global as any).defaultServiceNowConnection = { instanceUrl, username, password };
  ...
  private initializeLLMProviders(): void {
    ...
    for (const provider of providers) {
      const hasKey =
        process.env[provider.keyVar] ||
        process.env[`VITE_${provider.keyVar}`];
      
      const model =
        process.env[provider.modelVar] ||
   ...
   */
  getJiraConnection() {
    return (global as any).defaultJiraConnection;
  }
  ...
  getServiceNowConnection() {
    return (global as any).defaultServiceNowConnection;
  }
