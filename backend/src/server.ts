...
app.use((req: Request, _res: Response, next: NextFunction) => {
  if (req.session && !((req.session as any).jira) && connectionManager.isJiraConnected()) {
    const jiraConn = connectionManager.getJiraConnection();
    if (jiraConn) {
      (req.session as any).jira = jiraConn;
    }
  }

  if (req.session && !((req.session as any).servicenow) && connectionManager.isServiceNowConnected()) {
    const snConn = connectionManager.getServiceNowConnection();
    if (snConn) {
      (req.session as any).servicenow = snConn;
    }
  }
...
app.get('/api/jira/stories', async (req: Request, res: Response) => {
  try {
    let jiraSession = (req.session as any).jira;
...
app.get('/api/servicenow/stories', async (req: Request, res: Response) => {
  try {
    let snowSession = (req.session as any).servicenow;
 ...
 */
app.post('/api/test-cases/evaluate', async (req: Request, res: Response) => {
  try {
    ...
    const response = await axios.post(
      `${deepevalUrl}/api/test-cases/evaluate`,
      { testCases, userStory, metrics },
      { 
 ...
 */
app.post('/api/test-cases/generate-with-eval', async (req: Request, res: Response) => {
  try {
    const { story, numTestCases, provider, model } = req.body;
...
- Return ONLY the JSON object`;
    ...
    const testCasesForEval = parsed.test_cases.map((tc: any, index: number) => ({
      id: `TC-${index + 1}`,
      name: tc.name,
    ...
    const testCasesWithQuality = parsed.test_cases.map((tc: any, index: number) => {
      const evaluation = evaluations.find((e: any) => e.testCaseId === `TC-${index + 1}`);
      return {
        ...tc,
        id: `TC-${index + 1}`,
        quality: evaluation ? {
 ...
 */
app.post('/api/test-plan/generate', async (req: Request, res: Response) => {
  try {
 ...
 */
app.post('/api/test-cases/export', async (req: Request, res: Response) => {
  try {
      ...
      if (storyKey) {
        try {
          const storyResponse = await axios.get(
            `${baseUrl}rest/api/3/issue/${storyKey}`,
            {
              headers: {
                ...
                description: {
                  ...
                  content: [
                    ...
                    ...(testCase.steps && testCase.steps.length > 0 ? [
                      ...
                      {
                        type: 'orderedList',
                        content: testCase.steps.map((step: any) => ({
                          type: 'listItem',
                          content: [
                            {
                              type: 'paragraph',
      ...
      for (const testCase of testCases) {
        try {
          ...
          const testResponse = await axios.post(
            `${baseUrl}api/now/table/sn_test_management_test`,
            {
          ...
          const versionResponse = await axios.post(
            `${baseUrl}api/now/table/sn_test_management_test_version`,
            {
              test: testSysId,
          ...
          if (testCase.steps && testCase.steps.length > 0) {
            for (let i = 0; i < testCase.steps.length; i++) {
              ...
              await axios.post(
                `${baseUrl}api/now/table/sn_test_management_step`,
                {
          ...
          if (testCase.story_id) {
            try {
              await axios.post(
                `${baseUrl}api/now/table/sn_test_management_m2m_task_test`,
                {
              ...
              if (testCase.story_id) {
                try {
                  await axios.patch(
                    `${baseUrl}api/now/table/task/${testCase.story_id}`,
                    {
...
function mapPriorityToServiceNow(priority: string): string {
 ...
 */
app.get('/api/pii/config', (req: Request, res: Response) => {
  try {
    const config = (req.session as any)?.piiConfig;
 ...
 */
app.post('/api/pii/config', (req: Request, res: Response) => {
  try {
    ...
    (req.session as any).piiConfig = config;
 ...
 */
app.post('/api/pii/detect', (req: Request, res: Response) => {
  try {
    ...
    const piiConfig = config || (req.session as any)?.piiConfig || {
      mode: 'warn',
      sensitivityLevel: 'medium',
      enabledTypes: ['email', 'phone', 'ssn', 'creditCard', 'bankAccount', 'passport', 'dob', 'driverLicense', 'ipAddress'],
      autoSave: true,
